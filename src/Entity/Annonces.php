...
class Annonces
{
    ...
    public function addMandataire(Mandataire $mandataire): self
    {
        if (!$this->mandataires->contains($mandataire)) {
            $this->mandataires[] = $mandataire;
            $mandataire->setAnnonces($this);
        }

        return $this;
    }

    public function removeMandataire(Mandataire $mandataire): self
    {
        if ($this->mandataires->contains($mandataire)) {
            $this->mandataires->removeElement($mandataire);
            // set the owning side to null (unless already changed)
            if ($mandataire->getAnnonces() === $this) {
                $mandataire->setAnnonces(null);
            }
        }
